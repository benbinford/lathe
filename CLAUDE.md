...
cmd/
  ...
  skills.go, skills-install.go, skills-list.go    lathe skills install/list — write embedded skills to Claude Code / Cursor / Codex / Gemini / opencode / Cline / Windsurf (skills.go = parent cmd + rawShipTargets table)
...
internal/
  ...
  store/
    metadata.go                   Tutorial struct (incl. Repo/RepoBranch + Tool/Tools + Sources + Voice + Model), Status enum, Read/WriteMetadata, RepoDisplay, VerifyResult, Progress (Read/WriteProgress → progress.json sidecar)
  ...
  serve/
    ...
    progress.go, card_progress.go POST /-/progress endpoint; CardProgress model for list-card progress bars
    http.go                       readJSONBody (size-capped JSON body helper) + clampRatio
...
- **`internal/serve/server.go`** uses Go 1.22+ method-and-pattern routing (`mux.HandleFunc("GET /{slug}/", …)`). `safeTutorialPath` defends against path traversal by checking the joined path stays under `tutorialsDir` — preserve that check on any new route. State-changing POST routes are guarded by `sameOrigin` (rejects a present Origin/Referer that isn't local — CSRF defense for the predictable localhost port); apply it to any new POST handler. JSON request bodies go through `readJSONBody` (`http.go`), which size-caps and distinguishes oversize (413) from other read failures (400).
...
- **Reader progress** is an explicit save, not auto-tracking: the "Save progress" button in `layout.html` POSTs `{ratio, heading_id}` to `/-/progress/{slug}/{part}` (`progress.go`), persisted as a `progress.json` sidecar next to `metadata.json` via `store.WriteProgress`. `Tutorial.Progress` is tagged `json:"-"` and read best-effort at `ReadMetadata` time, so a ReadMetadata→mutate→WriteMetadata round-trip can never snapshot the sidecar into `metadata.json` — the binary stays the sole writer of each durable file, and a corrupt/missing `progress.json` never blocks reading the tutorial. The reading page restores the saved position client-side (scroll-ratio + optional heading-id hint, marker on the top progress bar); the list page renders per-card progress via `cardProgress` (`card_progress.go`) — a segmented bar for series, a single bar otherwise, and nothing if the saved part no longer exists in `Parts` (e.g. after a re-store).
